...
"""
    ...
    def _extract_main_script(self, text: str) -> str:
        ...
        main_script = self._extract_section(text, "=== MAIN_SCRIPT_START ===", "=== MAIN_SCRIPT_END ===")
        ...
        for pattern in patterns:
            ...
            for match in matches:
                if len(match.strip()) > 50 and ('use strict' in match or 'use warnings' in match):
                    return match.strip()
    ...
    def _extract_additional_files(self, text: str) -> List[Dict[str, str]]:
        ...
        for match in matches:
            ...
            for line in lines:
                ...
                elif not line.startswith('FILENAME:') and not line.startswith('DESCRIPTION:') and not line.startswith('TYPE:'):
            ...
            if content_lines:
                ...
                for i, line in enumerate(full_lines):
                    if not line.strip().startswith(('FILENAME:', 'DESCRIPTION:', 'TYPE:')):
                        content_start = i
                        break
    ...
    def _extract_security_measures(self, text: str) -> List[str]:
        ...
        security_text = self._extract_section(text, "=== SECURITY_MEASURES_START ===", "=== SECURITY_MEASURES_END ===")
        
        if not security_text:
            return ["Input validation implemented", "Error handling included", "Secure file operations"]
    ...
    def _extract_installation_steps(self, text: str) -> List[str]:
        ...
        install_text = self._extract_section(text, "=== INSTALLATION_START ===", "=== INSTALLATION_END ===")
        
        if not install_text:
            return ["Install required CPAN modules", "Set appropriate permissions", "Configure as needed"]
    ...
    def _extract_configuration(self, text: str) -> Dict[str, Any]:
        ...
        config_text = self._extract_section(text, "=== CONFIGURATION_START ===", "=== CONFIGURATION_END ===")
    ...
    def _extract_dependencies(self, text: str) -> Dict[str, List]:
        ...
        deps_text = self._extract_section(text, "=== DEPENDENCIES_START ===", "=== DEPENDENCIES_END ===")
        ...
        for line in lines:
            ...
            if line.startswith('Core Modules:'):
                ...
                if modules:
                    dependencies['core_modules'] = [m.strip() for m in modules.split(',')]
                    ...
                    for module_info in modules_text.split(','):
                        ...
                        if '(' in module_info:
                            ...
                            dependencies['cpan_modules'].append({
                                ...
                                'purpose': details.split(' - ')[-1] if ' - ' in details else 'Required module'
                            })
                        else:
                            dependencies['cpan_modules'].append({
                                'name': module_info,
    ...
    def _extract_deployment_info(self, text: str) -> List[str]:
        ...
        deployment_text = self._extract_section(text, "=== DEPLOYMENT_START ===", "=== DEPLOYMENT_END ===")
        
        if not deployment_text:
            return ["Deploy to appropriate server environment", "Set proper file permissions", "Configure monitoring"]
        ...
        return deployment_steps if deployment_steps else ["Follow standard deployment procedures"]
    ...
    def _parse_structured_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract description
            description = self._extract_section(response_text, "=== DESCRIPTION_START ===", "=== DESCRIPTION_END ===")
            if not description:
            ...
            parsed_data['documentation']['usage_examples'] = self._extract_usage_examples(response_text)
            parsed_data['documentation']['features'] = self._extract_features(response_text)
            parsed_data['documentation']['installation_steps'] = self._extract_installation_steps(response_text)
            parsed_data['documentation']['configuration'] = self._extract_configuration(response_text)
            parsed_data['code_structure']['functions'] = self._extract_functions(response_text)
            parsed_data['security']['measures'] = self._extract_security_measures(response_text)
            parsed_data['performance']['monitoring'] = self._extract_monitoring_info(response_text)
            parsed_data['deployment']['steps'] = self._extract_deployment_info(response_text)
            ...
            error_handling = self._extract_section(response_text, "=== ERROR_HANDLING_START ===", "=== ERROR_HANDLING_END ===")
            if error_handling:
                parsed_data['error_handling'] = [line.strip() for line in error_handling.split('\n') if line.strip()]
            
            # Extract notes
            notes = self._extract_section(response_text, "=== NOTES_START ===", "=== NOTES_END ===")
            if notes:
            ...
            console.print(f"🔍 [yellow]Error parsing structured response: {str(e)}[/yellow]")
            return {
                'status': 'error',
                'response_type': 'perl_code_generation',
                'metadata': {'model_used': self.model_name, 'complexity_level': 'unknown', 'estimated_lines': 0},
                'perl_code': {'main_script': '', 'additional_files': []},
                'dependencies': {'core_modules': [], 'cpan_modules': [], 'system_requirements': []},
                'documentation': {'description': 'Error parsing response', 'usage_examples': [], 'features': [], 'notes': []},
                'code_structure': {'functions': [], 'main_sections': []},
                'security': {'measures': []},
                'best_practices': [],
                'testing': {'test_cases': [], 'sample_input': '', 'expected_output': ''}
            }

    def _extract_usage_examples(self, text: str) -> List[str]:
        """Extract comprehensive usage examples"""
        usage_text = self._extract_section(text, "=== USAGE_EXAMPLES_START ===", "=== USAGE_EXAMPLES_END ===")
        ...
        for line in usage_text.split('\n'):
            line = line.strip()
            if line and not line.startswith('===') and ('perl' in line or line.startswith('./')):
    ...
    def _extract_features(self, text: str) -> List[str]:
        ...
        features_text = self._extract_section(text, "=== FEATURES_START ===", "=== FEATURES_END ===")
        
        if not features_text:
            return ["Modern Perl implementation", "Comprehensive error handling", "Security-focused design"]
    ...
    def _extract_functions(self, text: str) -> List[Dict[str, Any]]:
        ...
        functions_text = self._extract_section(text, "=== FUNCTIONS_START ===", "=== FUNCTIONS_END ===")
    ...
    def _extract_testing_info(self, text: str) -> Dict[str, Any]:
        ...
        testing_text = self._extract_section(text, "=== TESTING_START ===", "=== TESTING_END ===")
        ...
        for line in lines:
            ...
            if line.startswith('Test Case'):
                ...
                testing_info['expected_output'] = line.replace('Expected Output:', '').strip()
            elif line.startswith('Unit Tests:'):
                testing_info['unit_tests'].append(line.replace('Unit Tests:', '').strip())
            elif line.startswith('Integration Tests:'):
                testing_info['integration_tests'].append(line.replace('Integration Tests:', '').strip())
            elif line.startswith('Security Tests:'):
                testing_info['security_tests'].append(line.replace('Security Tests:', '').strip())
        
        return testing_info

    def _extract_best_practices(self, text: str) -> List[str]:
        """Extract comprehensive best practices"""
        practices_text = self._extract_section(text, "=== BEST_PRACTICES_START ===", "=== BEST_PRACTICES_END ===")
        ...
        return practices if practices else ["Enterprise-level best practices implemented"]

    def _generate_with_retry(self, prompt: str, max_retries: int = 3) -> Tuple[Optional[Dict], Optional[str]]:
...
"""
        ...
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    console.print(f"🔄 [yellow]Retry {attempt + 1}/{max_retries} - requesting comprehensive solution...[/yellow]")
                ...
                console.print(f"🤖 [yellow]Generating comprehensive solution (attempt {attempt + 1})...[/yellow]")
                ...
                if elapsed_time > 15:
                    console.print(f"⏱️ [blue]Model took {elapsed_time:.1f}s (generating comprehensive solution)[/blue]")
                
                if not response.text:
                    console.print(f"❌ [red]Attempt {attempt + 1}: No response from model[/red]")
                    continue
                ...
                main_script = parsed_response.get('perl_code', {}).get('main_script', '')
                additional_files = parsed_response.get('perl_code', {}).get('additional_files', [])
                
                if main_script and len(main_script.strip()) > 50:
                    console.print(f"✅ [green]Comprehensive solution generated on attempt {attempt + 1}[/green]")
                    if additional_files:
                        console.print(f"📁 [blue]Generated {len(additional_files)} additional files[/blue]")
                    return parsed_response, response.text
                else:
                    console.print(f"❌ [red]Attempt {attempt + 1}: Solution incomplete[/red]")
                    if attempt < max_retries - 1:
                        console.print(f"🔍 [yellow]Response preview: {response.text[:200]}...[/yellow]")
                        time.sleep(2)
    ...
    def generate_code(self, prompt: str, save_to_file: Optional[bool] = None, filename: Optional[str] = None) -> Tuple[Optional[Dict], Optional[str], Optional[Dict]]:
        ...
        try:
            ...
            console.print("🔒 [green]Enterprise-level security and best practices enabled[/green]")
            
            parsed_response, raw_response = self._generate_with_retry(prompt, max_retries=3)
            ...
            if parsed_response and parsed_response.get('status') == 'success':
                ...
                console.print("❌ [red]Failed to generate comprehensive solution after 3 attempts[/red]")
                if raw_response:
                    console.print("📄 [yellow]Last raw response:[/yellow]")
                    console.print(Panel(raw_response[:500] + "...", title="Raw Response (Truncated)", border_style="yellow"))
                return None, None, None
                
        except Exception as e:
            console.print(f"❌ [red]Error generating comprehensive code: {str(e)}[/red]")
            return None, None, None
    ...
    def stream_generate(self, prompt: str) -> Optional[Dict]:
        ...
        try:
            ...
            console.print(f"🤖 [yellow]clicod streaming comprehensive solution from {self.model_name}...[/yellow]")
            start_time = time.time()
            ...
            if parsed_response and parsed_response.get('status') == 'success':
                console.print("\n" + "="*80)
                console.print("📊 [bold green]Successfully parsed comprehensive solution![/bold green]")
                self._render_comprehensive_output(parsed_response)
                return parsed_response
            else:
                console.print("❌ [red]Failed to parse comprehensive streamed response[/red]")
                console.print("🔄 [yellow]Attempting to fix with retry logic...[/yellow]")
    ...
    def _render_comprehensive_output(self, parsed_response):
        ...
        if documentation.get('description'):
            ...
            description = re.sub(r'#{1,6}\s*([^\n]+)', r'\1', description)  # Remove headers
        ...
        if additional_files:
            ...
            for i, file_info in enumerate(additional_files, 1):
                ...
                console.print(Panel(
                    Syntax(file_info.get('content', ''), 
                          "perl" if file_info.get('filename', '').endswith('.pl') else "text", 
                          theme="monokai"),
        ...
        if security.get('measures'):
            security_text = "\n".join([f"🔒 {measure}" for measure in security['measures']])
            console.print(Panel(
                security_text,
        ...
        if configuration:
            ...
            if configuration.get('environment_vars'):
                ...
                config_text += "\n".join([f"  • {var}" for var in configuration['environment_vars']])
            if configuration.get('options'):
                config_text += "\nOptions:\n"
                config_text += "\n".join([f"  • {opt}" for opt in configuration['options']])
    ...
    def _render_comprehensive_dependencies(self, dependencies):
        ...
        for module in dependencies.get('security_modules', []):
            table.add_row("Security", module, f"cpan install {module}", "Security enhancement")
        
        # Development tools
        for tool in dependencies.get('development_tools', []):
            table.add_row("Dev Tool", tool, f"cpan install {tool}", "Development support")
    ...
    def _render_comprehensive_testing_info(self, testing):
        ...
        if testing.get('integration_tests'):
            ...
            test_content += "\n".join([f"• {test}" for test in testing['integration_tests']])
            test_content += "\n\n"
        
        if testing.get('security_tests'):
            test_content += "Security Tests:\n"
            test_content += "\n".join([f"• {test}" for test in testing['security_tests']])
            test_content += "\n\n"
    ...
    def _save_comprehensive_code(self, parsed_response, filename=None):
        ...
        if not filename:
            filename = Prompt.ask("Enter main script filename", default="clicod_enterprise.pl")
        ...
        try:
            ...
            with open(filepath, 'w') as f:
                ...
                if install_steps:
                    f.write("#\n# Installation:\n")
                    for i, step in enumerate(install_steps[:5], 1):  # Limit to first 5 steps
                        f.write(f"# {i}. {step}\n")
            ...
            for file_info in additional_files:
                ...
                console.print(f"✅ [green]{file_info.get('type', 'File').title()} saved: {add_filepath}[/green]")
            ...
            all_cpan_modules = dependencies.get('cpan_modules', []) + dependencies.get('security_modules', []) + dependencies.get('development_tools', [])
            if all_cpan_modules:
            ...
            console.print(f"❌ [red]Error saving comprehensive solution: {str(e)}[/red]")
